...
import os, time, datetime
import selenium_login as login 
...
bookinfo1 = {
    ...
    'date':'2021년 10월',
    'price':'30,000',
    'img':os.path.abspath('./ch4/input/book_img1.jpg')
    }
bookinfo2 = {
    'title':'유튜브 영상 편집을 위한\파이널 컷 프로 X',
    'date':'2021년 09월',
    'price':'29,800',
    'img':os.path.abspath('./ch4/input/book_img2.jpg')
    }
...
def count_book(driver):
   # div = driver.find_element_by_id('cnt') #--- (7a)
    div = driver.find_element(By.ID, 'cnt')
    num = div.text
    print('현재 '+num+'권')
    return int(num) #--- (7b)
...
def add_book(driver, bookinfo):
    ...
    driver.find_element(By.CSS_SELECTOR, 'label.mainbtn').click()
    driver.find_element(By.NAME, 'ntitle').send_keys(bookinfo['title'])
    ...
    driver.find_element(By.CSS_SELECTOR, 'label.subbtn').click() 
